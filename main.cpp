...
int main(int argc, char *argv[])
{
	...
	try
	{
		...
		glob(images_path_cv, filenames_in_folder_cv,true); // void glob(String pattern, std::vector<String>& result, bool recursive = false);
		...
		for (auto &i : filenames_in_folder)
		{
			...
			images_path = i.substr(0,pos_filename-1);
			std::string const ext = i.substr(i.find_last_of(".") + 1);
			std::string const filename_without_ext = i.substr(0, i.find_last_of("."));
					...
					ext == "ppm" || ext == "PPM")
			{
				...
				if(std::find(jpg_filenames_without_ext.begin(), jpg_filenames_without_ext.end(), filename_without_ext) == jpg_filenames_without_ext.end()){
					Mat image = imread(i, 1);
					imwrite(filename_without_ext+".jpg", image);
					jpg_filenames_without_ext.push_back(filename_without_ext);
					image_ext.push_back("jpg");
					jpg_filenames.push_back(filename_without_ext+".jpg");
					jpg_filenames_path.push_back(i);
				}
			}
		}
		...
		if (jpg_filenames.size() == 0) {
			...
			std::cout << "Loaded " << jpg_filenames.size()<< " images." << std::endl;
		}
		...
		std::cout << "Loaded " << intersect_filenames.size()<< " images." << std::endl;
		for (size_t i = 0; i < intersect_filenames.size(); ++i) {
			ofs_train << intersect_filenames[i] << "." << intersect_ext[i] << std::endl;
		}
		ofs_train.flush();
		...
		do {
			...
			if (old_trackbar_value != trackbar_value || exit_flag)
			{
				...
				if (old_trackbar_value >= 0) // && current_coord_vec.size() > 0) // Yolo v2 can processes background-image without objects
				{
					try
					{
						...
						std::string const ext = jpg_filename.substr(jpg_filename.find_last_of(".") + 1);
						std::string const filename_without_ext = jpg_filenames_without_ext[old_trackbar_value];
						std::string const txt_filename = filename_without_ext + ".txt";

						std::cout << "txt_filename_path = " << txt_filename << std::endl;

						std::ofstream ofs(txt_filename, std::ios::out | std::ios::trunc);
						ofs << std::fixed;
						...
						{
							ofs_train << filename_without_ext + ext << std::endl;
							ofs_train.flush();
				...
				for (size_t i = 0; i < preview_number && (i + trackbar_value) < jpg_filenames_path.size(); ++i)
				{
					...
					if (i == 0)
					{
						...
						try {
							...
							std::string const txt_filename = jpg_filenames_without_ext[trackbar_value] + ".txt";
...

			{
				...
				putText(full_image_roi, obj_str, Point2i(0, 21), FONT_HERSHEY_DUPLEX, 0.6, Scalar(10, 50, 10), 3);
				putText(full_image_roi, obj_str, Point2i(0, 21), FONT_HERSHEY_DUPLEX, 0.6, Scalar(20, 120, 60), 2);
				putText(full_image_roi, obj_str, Point2i(0, 21), FONT_HERSHEY_DUPLEX, 0.6, Scalar(50, 200, 100), 1);
			}
			...
			if (pressed_key == 27 || pressed_key == 1048603) exit_flag = true;// break;  //  - save & exit
